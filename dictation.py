...
def main():
    ...
    if args.command == 'wb':
        if args.action in {'add', 'remove'}:
            ...
            # 打乱顺序
            df_refs = df_refs.sample(frac=1).reset_index(drop=True)
    ...
    # sample 模式去重逻辑
    sampled_refs_path = os.path.join(args.output, 'sampled_refs.txt')
    os.makedirs(args.output, exist_ok=True)
    sampled_refs = set()
    if args.mode == 'sample':
        if os.path.exists(sampled_refs_path):
            with open(sampled_refs_path, 'r', encoding='utf-8') as f:
                sampled_refs = set(line.strip() for line in f if line.strip())
        # 当前可抽样的单词
        df_filtered['ref_str'] = df_filtered.apply(lambda r: f"{r['list_no']}-{r['index']}", axis=1)
        df_available = df_filtered[~df_filtered['ref_str'].isin(sampled_refs)].reset_index(drop=True)
        available_count = len(df_available)
        if available_count == 0:
            print('所有单词已抽取过，无可用单词。')
            return
        if args.count > available_count:
            print(f"仅剩 {available_count} 个单词可抽取，将全部输出。")
            df_final = df_available
        else:
            df_final = df_available.sample(n=args.count).reset_index(drop=True)
        # 记录本次抽取的编号
        new_refs = set(df_final['ref_str'].tolist())
        sampled_refs.update(new_refs)
        with open(sampled_refs_path, 'w', encoding='utf-8') as f:
            for ref in sorted(sampled_refs, key=lambda x: (int(x.split('-')[0]), int(x.split('-')[1]))):
                f.write(ref + '\n')
        # 去掉辅助列
        df_final = df_final.drop(columns=['ref_str'])
    else:
